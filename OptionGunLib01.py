...
def BuildOptionMetrics(all_options, FEE_SPREAD, HIDE_TICKERS, DATE_ONLY_FORMAT, DATE_TIME_FORMAT):
    ...
    all_options['Quote_Time'] = pd.to_datetime(all_options['Quote_Time'], format = DATE_TIME_FORMAT, errors = 'coerce')
    all_options['Expiry']     = pd.to_datetime(all_options['Expiry'], format = DATE_ONLY_FORMAT, errors = 'coerce')
    ...
    #all_options['ER Date']    = pd.to_datetime(all_options['ER Date'], format = DATE_FORMAT, errors = 'coerce')
